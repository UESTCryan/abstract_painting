...
def conv2d(x, input_filters, output_filters, kernel, strides, mode='REFLECT'):
    with tf.compat.v1.variable_scope('conv') as scope:
        ...
        weight = tf.Variable(tf.compat.v1.truncated_normal(shape, stddev=0.1), name='weight')
...
def conv2d_transpose(x, input_filters, output_filters, kernel, strides):
    with tf.compat.v1.variable_scope('conv_transpose') as scope:
...
def resize_conv2d(x, input_filters, output_filters, kernel, strides, training):
    ...
    with tf.compat.v1.variable_scope('conv_transpose') as scope:
        ...
        x_resized = tf.image.resize(x, [new_height, new_width], tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def instance_norm(x):
    ...
    return tf.divide(tf.subtract(x, mean), tf.sqrt(tf.add(var, epsilon)))
...
def residual(x, filters, kernel, strides):
    with tf.compat.v1.variable_scope('residual') as scope:
...
def transform_network(image, training):
    ...
    with tf.compat.v1.variable_scope('res3'):
        res3 = residual(res2, 128, 3, 1)
    with tf.compat.v1.variable_scope('res4'):
        res4 = residual(res3, 128, 3, 1)
    with tf.compat.v1.variable_scope('res5'):
